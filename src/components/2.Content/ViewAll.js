...
function ViewAll() {
    ...
    const ProductResultA = new Set(ProductResult)
    const ProductResult2 = [...ProductResultA]
    ...
    if (callApiSuccess) {
        const phanTrang = () => {
            // Tính tổng số trang
            totalPage = Math.ceil(ProductResult2.length / 12)
            // Tính số trang hiện tại từ dòng bao nhiêu đến bao nhiêu
            let n1 = nPage;
            let b1 = n1 * 12 - 12
            let b2 = n1 * 12 - 1
            ProductResult2.forEach((item, index) => {
    ...
    return (
        <Container className="containerViewAll" style={{ height: "75vh" }} id="marginTopViewAll">
            <Breadcrumbs aria-label="breadcrumb" className="mt-3">
                <Link underline="hover" color="inherit" href={siteProps[0].url} style={{ fontFamily: "Montserrat" }}>
                    {siteProps[0].name}
                </Link>
                <Typography color="text.primary" style={{ fontFamily: "Montserrat" }}>Tất cả sản phẩm</Typography> <a href={siteProps[0].url} style={{ fontFamily: "Montserrat" }}>Back</a>
            </Breadcrumbs>
            {
